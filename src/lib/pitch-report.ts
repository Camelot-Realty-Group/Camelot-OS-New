...
function davidGoldoffClientSignatureHtml(): string {
  // Compact block: the previous 250px signature + 14px top margin pushed the
  // company line past the letter card's max-height, clipping the sign-off.
  return `<div style="font-family:'Plus Jakarta Sans',Arial,sans-serif;font-size:12px;color:#1a2744;line-height:1.3"><strong>Sincerely yours,</strong><div style="margin-top:6px"><img src="${DAVID_GOLDOFF_SIGNATURE_IMAGE}" alt="David Goldoff signature" style="display:block;width:200px;max-width:52%;height:auto;margin:0 0 4px 0"><div style="font-size:13px;font-weight:800">David A. Goldoff</div><div style="font-size:11px;color:#333">President, Camelot Property Management Services Corp.</div></div></div>`;
}
...
function preparedForBlock(d: MasterReportData): string {
  ...
  // Keep it simple and human: a known contact, a known ownership entity, or
  // just "the Ownership of {address}" — never a data-source label like
  // "Property Owner (DOF)".
  const entity = ownerEntityName(d);
  const preparedFor = name || (entity ? `the Ownership of ${entity}` : `the Ownership of ${d.buildingName || d.address}`);
...
function ownerEntityName(d: MasterReportData): string {
  let name = String((d as any).registrationOwner || (d as any).dofOwner || '').trim();
  if (!name || /department|dcas|city of new york|housing authority|nycha|state of new york/i.test(name)) return '';
  // Tax-roll suffixes like "LIBERTY VIEW CONDOMINIUM.ASSOC" are cleaned into
  // human form ("Liberty View Condominium") for letters and covers.
  name = name.replace(/[.,]?\s*ASSOC(IATION)?\.?$/i, '').replace(/\s+/g, ' ').trim();
  return name.split(' ').map(w => /^(llc|corp|inc|hdfc)\.?$/i.test(w) ? w.toUpperCase() : w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(' ');
}
...
function directionsEmbedUrl(d: MasterReportData, mode: 'driving' | 'transit' = 'driving'): string {
  return `https://www.google.com/maps/embed/v1/directions?key=${GOOGLE_MAPS_KEY}&origin=57+West+57th+Street+Suite+410+New+York+NY+10019&destination=${encodeURIComponent(d.address)}&mode=${mode}`;
}

/** Street portion of the address only: "125 Walker Street, New York, NY" → "125 Walker Street". */
...
function contextualImageCard(d: MasterReportData, index: number, caption: string, height = 220): string {
  ...
  const safeAlt = escapeHtml(`${d.buildingName || d.address} property photo`);
  if (photos.length) {
    // Lead with the uploaded/verified photo set. If a photo URL ever fails,
...
function siteSatelliteEmbedUrl(d: MasterReportData): string {
  const q = d.latitude && d.longitude ? `${d.latitude},${d.longitude}` : `${d.address}, New York, NY`;
  return `https://www.google.com/maps/embed/v1/place?key=${GOOGLE_MAPS_KEY}&q=${encodeURIComponent(q)}&zoom=18&maptype=satellite`;
}
...
function localServicesCard(d: MasterReportData, height = 220): string {
  return iframeCard(siteSatelliteEmbedUrl(d), 'Property site satellite view', `Satellite close-up — ${d.buildingName || d.address} site location`, height);
}
...
function propertyImageCard(d: MasterReportData, caption: string, height = 392): string {
  ...
  // bestExteriorImage() already reflects building-photos.ts's full fallback
  // chain (Places -> Wikimedia -> aimed Street View -> nothing), so an aimed
  // shot is already in `first` whenever one was found. Do NOT bolt on a
  // second, unaimed (heading=0) Street View fallback here -- that's what
  // put the wrong building in these reports before.
  ...
  return first
    ? `<div class="visual-card"><div class="image-frame" style="height:${height}px"><img src="${first}" alt="${safeAlt}" onerror="this.style.display='none';this.parentElement.innerHTML='${placeholder}'"></div><div class="image-caption">${caption}</div></div>`
    : `<div class="visual-card"><div class="image-frame" style="height:${height}px">${placeholder}</div><div class="image-caption">${caption}</div></div>`;
}

function noExteriorPhotoCard(_unusedSrc: string, title: string, caption: string, height = 260): string {
  // Same reasoning as propertyImageCard above: an aimed Street View shot
  // (when building-photos.ts found one) already comes through as a real
  // photo via bestExteriorImage/propertyPhotoStack, so callers only reach
  // this function when nothing legitimate exists at all. Show the honest
  // placeholder rather than a second, unaimed Street View guess.
  const safeTitle = escapeHtml(title);
  return `<div class="visual-card"><div class="image-frame" style="height:${height}px;display:flex;align-items:center;justify-content:center;background:#F5F0E5;color:#8a8174;font-size:11px;font-weight:700;text-align:center;padding:12px">No exterior photo available for ${safeTitle}</div><div class="image-caption">${caption}</div></div>`;
}
...
<div class="slide"><div class="pad" style="padding:40px 64px">${logoBadge()}<div class="section-title" style="margin-bottom:13px">Property Snapshot &amp; New York Reach</div><div style="display:grid;grid-template-columns:.9fr 1.1fr;gap:15px"><div><div class="gold-card" style="margin-bottom:8px;padding:12px 14px"><div class="sub-heading">Property Snapshot</div><table style="font-size:12px"><tr><td>Owner / Entity</td><td>${escapeHtml(ownerEntityName(d) || String((d as any).dofOwner || '') || 'To verify (HPD / ACRIS)')}</td></tr><tr><td>Type</td><td>${d.propertyType || 'Residential'}</td></tr><tr><td>Units</td><td>${unitsBreakdownLabel(d)}</td></tr><tr><td>Stories / Built</td><td>${d.stories ? `${d.stories} stories` : 'Stories: verify'} &middot; ${d.yearBuilt || 'year: verify'}</td></tr><tr><td>Block / Lot</td><td>${bblLabel(d)}</td></tr><tr><td>Neighborhood</td><td>${neighborhoodName(d)}</td></tr><tr><td>Current Management</td><td>${d.managementCompany ? `${escapeHtml(d.managementCompany)} <span style="font-size:10px;color:#888">(HPD registration &mdash; verify)</span>` : 'To verify &mdash; HPD / DOB / PropertyShark'}</td></tr></table></div><div class="gold-card" style="margin-bottom:8px;padding:12px 14px"><div class="sub-heading">Talking Points for Our First Conversation</div><p class="body-text" style="font-size:12.5px;line-height:1.45">${hasRisks ? `When we speak, we would walk you through what the public record shows today &mdash; ${d.violationsOpen} open HPD violation(s), ${fmt$(d.ecbPenaltyBalance)} in ECB penalty balance, and ${d.ll97?.period1Penalty ? fmt$(d.ll97.period1Penalty) + ' in modeled LL97 exposure' : 'LL97 status to confirm'} &mdash; and exactly how we monitor, resolve, and prevent items like these for the buildings we manage.` : `Your building's public record is comparatively clean. When we speak, we would talk about keeping it that way while tightening financial reporting, vendor pricing, and resident service &mdash; and where we typically find savings and new revenue.`} This page is a sample of the day-one visibility our platform gives ownership.</p></div></div><div style="display:grid;grid-template-columns:1fr;gap:8px;align-content:start">${propertyImageCard(d, `${d.buildingName || d.address} &mdash; street-level property view`, 128)}${iframeCard(siteSatelliteEmbedUrl(d), 'Property site satellite view', `Satellite close-up &mdash; ${d.buildingName || d.address} site location`, 96)}<div class="gold-card" style="padding:9px 12px;margin:0"><div class="sub-heading" style="font-size:14px;margin-bottom:4px">Nearby Commuting &amp; Entry Points</div>${(() => {
      ...
      const rows = (subs && subs.length)
        ? subs.slice(0, 2).map(s => `${s.name} (${s.routes.split(/\s+/).join('/')}) &middot; ${s.miles.toFixed(2)} mi`)
        : ['Nearest subway confirmed against MTA at onboarding'];
      rows.push('Bus, e-bike, rideshare &amp; taxi points mapped at onboarding');
      return rows.map(r => `<div class="check" style="font-size:10.5px;margin-bottom:4px;line-height:1.3"><span>&#10003;</span><div>${r}</div></div>`).join('');
    })()}</div></div></div><div class="source-note">Sources: Google Maps route, neighborhood context, Camelot property intelligence, and uploaded/verified property assets.</div></div></div>
...
export function generatePitchReport(d: MasterReportData): string {
  ...
  // THINKING LOGIC: every fact discovered about the building becomes a
  // specific service angle in the pitch narrative — the deck proposes what
  // THIS building needs, not generic management copy.
  const hookLines: string[] = [];
  if (d.violationsOpen > 0) hookLines.push(`${d.violationsOpen} open HPD violation${d.violationsOpen === 1 ? '' : 's'} — Camelot opens a cure path and re-inspection calendar in week one`);
  if (hasLL97) hookLines.push(`LL97 carbon penalty exposure starting 2030 — we model the fine, then engineer it down`);
  if (d.ecbPenaltyBalance > 0) hookLines.push(`${fmt$(d.ecbPenaltyBalance)} in outstanding ECB penalties compounding with interest — always our first savings target`);
  if (d.permitsCount > 3) hookLines.push(`${d.permitsCount} active permits — capital work needs owner-side oversight, not just contractor promises`);
  if ((d as any).facadeIssueCount > 0) hookLines.push(`LL11/FISP filings show SWARMP or unsafe status — sidewalk-shed and remediation cost control becomes urgent`);
  if ((d as any).isRentStabilized) hookLines.push(`Rent-stabilized units on record — DHCR registration and RGB compliance handled by our in-house specialists`);
  if ((d as any).hasActiveLitigation) hookLines.push(`Active housing litigation on record — we coordinate counsel, evidence, and agency response as one workstream`);
  if ((d as any).taxLienRecordCount > 0) hookLines.push(`Tax-lien sale records found — arrears strategy and DOF engagement before the next lien sale list`);
  if ((d as any).energyStarScore && (d as any).energyStarScore < 50) hookLines.push(`Energy Star ${(d as any).energyStarScore} — utility-cost recovery and benchmarking gains are on the table`);
  if (d.yearBuilt && d.yearBuilt < 1940) hookLines.push(`A ${d.yearBuilt}-vintage building — preventive-maintenance discipline beats emergency-repair pricing`);
